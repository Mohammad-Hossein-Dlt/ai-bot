...
from src.usecases.bot.content_creation.cache import ProduceContentCache
from src.usecases.bot.content_creation.steps.choose_ai_platform import ChooseAiPlatform
from src.usecases.bot.content_creation.steps.choose_model import ChooseModel
from src.usecases.bot.content_creation.steps.choose_prompt import ChoosePrompt
from src.usecases.bot.content_creation.steps.choose_words_number import ChooseWordsNumber
from src.usecases.bot.content_creation.steps.choose_tones import ChooseTones
from src.usecases.bot.content_creation.steps.request_summary import RequestSummary
from src.usecases.bot.content_creation.steps.produce import Produce
...
async def request_steps(
    ...
    cache_usecase = ProduceContentCache(
        cache_repo,
        user_repo,
        category_repo,
        prompts_titles,
    ...
    request: ContentCreationRequestModel = await cache_usecase.execute(chat_id, callback_data)
    ...
    if start_point:
        ...
        choose_prompt_usecase = ChoosePrompt(
            prompts_titles,
            inline_keyboard,
        )
        
        text, keyboard = choose_prompt_usecase.execute(callback_data)
        ...
        choose_words_number_usecase = ChooseWordsNumber(
            words_number,
            inline_keyboard,
        )
        
        text, keyboard = choose_words_number_usecase.execute(callback_data)
        ...
        choose_tones_usecase = ChooseTones(
            tones,
            inline_keyboard,
        )
        
        text, keyboard = choose_tones_usecase.execute(callback_data, request)
