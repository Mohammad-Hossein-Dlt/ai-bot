...
class ChooseAiActionType:
    ...
    def execute(
        ...
        for index in callback_data.paginate:
            if index >= len(self.ai_action_type_fa): break
            self.inline_keyboard.add_button(
                Button(
                    text=self.ai_action_type_fa[index],
                    callback_data=callback_data.encode(step=self.step+1, page=0, origin="aatfa", index=index),
                ),
            )
        ...
        if callback_data.start > 0 and callback_data.end < len(self.ai_action_type_fa):
            self.inline_keyboard.add_row(previous_page, next_page)
        elif callback_data.start > 0:
            self.inline_keyboard.add_row(previous_page)
        elif callback_data.end < len(self.ai_action_type_fa):
